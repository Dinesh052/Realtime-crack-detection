...
class RootWidget(TabbedPanel):
    ...
    def select_to(self, *args):
        try:
            ...
            cv2.imwrite(
                'E:\EL\crack-detection-beproject-master\GUI\original_im.jpg',
                original_image)
            result = main_function(image_gray)
            cv2.imwrite(
                'E:\EL\crack-detection-beproject-master\GUI\processed_im.jpg',
                result)
            ...
            for contour in contours:
                # Draw a green contour around the crack
                cv2.drawContours(detected_cracks_image, [contour], -1, (0, 255, 0), 2)

                # Get the coordinates of the bounding box
                x, y, w, h = cv2.boundingRect(contour)

                # Draw a red bounding box around the crack
                cv2.rectangle(detected_cracks_image, (x, y), (x + w, y + h), (0, 0, 255), 2)

            # Save the image with detected contours and bounding boxes
            cv2.imwrite(
                'E:\EL\crack-detection-beproject-master\GUI\detected_cracks.jpg',
                detected_cracks_image)

            self.img3.source = './processed_im.jpg'
            self.img4.source = './detected_cracks.jpg'  # Show the image with detected cracks and bounding boxes
            ...
            if crack_detected:
                ...
                # Send an email with image and metadata
                subject = "Crack Detected"
                body = "A crack has been detected in the image. Metadata:\n"
                body += f"Image Path: {args[1][0]}\n"
                body += f"Number of Contours (Cracks): {len(contours)}\n"
